...
# Exec Plan: 行路編集 v0（検証OFF・右クリック挿入・便D&D・地図モーダル）

## 全体像

行路（Block）編集に特化した最小UIを提供する。検証（サービス日/時刻整合）は当面OFFで、操作自由度を優先。便右クリックで休憩/回送を「前/後」に挿入、便ドラッグ＆ドロップで「分離→結合」を無条件で実行。便から route を辿って地図モーダルで表示。区間は blocks_intervals.csv に保存（note 列あり）。
...
- 既定フローで UI スナップショット実行。失敗時は `tmp/ui-snapshots/fallback-*.md` のログパスを本 plans.md に追記。
